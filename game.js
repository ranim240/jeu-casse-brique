...
function createBricks() {
  for (let r = 0; r < brickProp.row; r++) {
    bricks[r] = [];
    let coloredCount = 0; // Counter for colored bricks
    for (let c = 0; c < brickProp.column; c++) {
      if (coloredCount < 2) {
        // Assign red color to the first two bricks in each row
        bricks[r][c] = {
        ...
        // Assign white color to the remaining bricks
        bricks[r][c] = {
  ...
  bricks.forEach((column) => {
    column.forEach((brick) => {
      if (brick.status) {
        ...
        // Ajouter une bordure autour de la brique
        ctx.strokeStyle = brick.strokeColor;
        ctx.lineWidth = brick.strokeWidth;
        ctx.stroke();
    ...
    column.forEach((brick) => {
      if (brick.status) {
        if (
          ...
          // Check if the brick is colored
          if (brick.color !== "#fff") {
            paddle.w *= 1.5; // Increase paddle size by 1.5 times
            ball.velocity += 2; // Increase ball speed
          }
        }
      }
    });
